...
class M_website extends CI_Model {
	...
	public function logo($width="200px", $height="50px"){
	    ...
	    unset($gambar[0]);
	    $src = implode('/', $gambar);
		return '<img style="max-width:'.$width.'; max-height:'.$height.'" src="'.base_url().$src.'"/>';
	}
        ...
        if($table=='barang') {
            ...
            foreach($get_data as $i => $item){
                ...
                if ($get_diskon != null) {
                    ...
                    $get_data[$i]['diskon2'] = $get_diskon['diskon2'];
    ...
    public function log_master($last_q, $lokasi) {
        $kode_log = $this->m_website->kode_log($lokasi);
        $this->m_crud->create_data("master_log", array(
            'id_master_log' => $kode_log,
            'tanggal' => date("Y-m-d H:i:s"),
            'query' => "insert into master_log values ('".$kode_log."SRV', current_timestamp, '".str_replace('\'','\'\'', $last_q)."', 'send', 'TRIGGER')",
            'param' => 'send',
            'lokasi' => $lokasi
        ));
    }
